...
export default {
  ...
  async fetch() {
    ...
    const _hash = {
      ...
      pod: this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: POD }),
      // restore: this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.RESTORE }),
    };

    if (this.$store.getters[`${ PRODUCT_NAME }/schemaFor`](NODE)) {
      _hash.nodes = this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: NODE });
      this.hasNode = true;
    }

    if (this.$store.getters[`${ PRODUCT_NAME }/schemaFor`](HCI.NODE_NETWORK)) {
      _hash.nodeNetworks = this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.NODE_NETWORK });
    }

    if (this.$store.getters[`${ PRODUCT_NAME }/schemaFor`](HCI.CLUSTER_NETWORK)) {
      _hash.clusterNetworks = this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.CLUSTER_NETWORK });
    }
  ...
  async created() {
    // const inStore = this.$store.getters['currentProduct'].inStore;
    const vmis = await this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.VMI });

    await this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.VMIM });
