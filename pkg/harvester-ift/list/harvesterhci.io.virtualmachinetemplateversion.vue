...
export default {
  ...
  async fetch() {
    ...
    const hash = await allHash({
      template:        this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.VM_TEMPLATE }),
      templateVersion: this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.VM_VERSION }),
    });
    ...
    <template #group-by="group">
      <div class="group-bar">
        <div class="group-tab">
          <div
            class="project-name"
            v-html="templateLabel(group.group)"
          />
        </div>
