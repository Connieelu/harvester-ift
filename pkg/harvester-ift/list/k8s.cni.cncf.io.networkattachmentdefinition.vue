...
export default {
  ...
  async fetch() {
    ...
    const _hash = { rows: this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: NETWORK_ATTACHMENT }) };

    if (this.$store.getters[`${ PRODUCT_NAME }/schemaFor`](HCI.NODE_NETWORK)) {
      _hash.hostNetworks = this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.NODE_NETWORK });
    }
