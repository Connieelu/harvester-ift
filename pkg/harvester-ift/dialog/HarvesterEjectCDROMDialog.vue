...
export default {
  ...
  methods: {
    ...
    remove(buttonDone) {
      try {
        this.actionResource.metadata.annotations[HCI_ANNOTATIONS.PVC_EJECT] = JSON.stringify(this.diskNames);
        this.actionResource.save()
          .then((value) => {
            value.doAction('ejectCdRom', { diskNames: this.diskNames });
          })
          .then(() => {
            this.close();
            buttonDone(true);
