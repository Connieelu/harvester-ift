...
export default {
  ...
  async fetch() {
    this.allPVCs = await this.$store.dispatch('cluster/findAll', { type: PVC });
  },
  ...
  computed: {
    ...
    volumeOption() {
      return sortBy(
        this.PVCs
          .filter( (pvc) => {
            const pvctype = pvc?.metadata?.annotations?.[HCI_ANNOTATIONS.VOLUME_VM] ? JSON.parse(pvc?.metadata?.annotations?.[HCI_ANNOTATIONS.VOLUME_VM]).type : '';

            if (!!pvc.metadata?.annotations?.[HCI_ANNOTATIONS.IMAGE_ID] || pvctype !== 'disk' ) {
              return false;
            }
