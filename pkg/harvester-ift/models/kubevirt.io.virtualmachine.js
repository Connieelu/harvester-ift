...
export default class VirtVm extends HarvesterResource {
  ...
  get podResource() {
    ...
    const vmiResource = this.$rootGetters[`${ PRODUCT_NAME }/byId`](HCI.VMI, this.id);
    const podList = this.$rootGetters[`${ PRODUCT_NAME }/all`](POD);
  ...
  get vmi() {
    ...
    const vmis = this.$rootGetters[`${ PRODUCT_NAME }/all`](HCI.VMI);
  ...
  restoreState(vmResource = this, id) {
    ...
    const allRestore = this.$rootGetters[`cluster/all`](HCI.RESTORE);
  ...
  get resourcesStatus() {
    // const inStore = this.productInStore;
    const vmList = this.$rootGetters[`${ PRODUCT_NAME }/all`](HCI.VM);
    let warningCount = 0;
    let errorCount = 0;
  ...
  get rootImageId() {
    let imageId = '';
    // const inStore = this.productInStore;
    const pvcs = this.$rootGetters[`${ PRODUCT_NAME }/all`](PVC) || [];
