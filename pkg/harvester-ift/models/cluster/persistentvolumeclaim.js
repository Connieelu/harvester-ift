...
export default class HciPv extends HarvesterResource {
  ...
  get stateDisplay() {
    // const ownedBy = this?.metadata?.annotations?.[HCI_ANNOTATIONS.VOLUME_CLONE_INFO];
    const pvcCloneInfo = this.metadata.annotations?.[HCI_ANNOTATIONS.VOLUME_CLONE_INFO] ? JSON.parse(this.metadata.annotations?.[HCI_ANNOTATIONS.VOLUME_CLONE_INFO]) : '';
    const pvcOccupation = pvcCloneInfo?.ifoccupied;

    const volumeError = this.relatedPV?.metadata?.annotations?.[HCI_ANNOTATIONS.VOLUME_ERROR];
    const pvcCloneStatus = this?.metadata?.annotations?.[HCI_ANNOTATIONS.PVC_CLONE_STATUS] ? JSON.parse(this.metadata.annotations[HCI_ANNOTATIONS.PVC_CLONE_STATUS]) : '';

    if (pvcCloneStatus?.ifrunning === 'True') {
      return 'Not Ready';
    }
    ...
    if (findBy(conditions, 'type', 'Resizing')?.status === 'True') {
      return 'Resizing';
    } else if (pvcOccupation === 'True' && !volumeError) {
      return 'In-use';
  ...
  get doneOverride() {
    ...
    // if (detailLocation.params.product === 'explorer') {
    //   return detailLocation;
    // }
  ...
  get attachVM() {
    const allVMs = this.$rootGetters[`${ PRODUCT_NAME }/all`](HCI.VM);
    const ownedBy =
  ...
  get longhornVolume() {
    ...
    return this.$rootGetters[`${ PRODUCT_NAME }/all`](LONGHORN.VOLUMES).find((v) => v.metadata?.name === this.spec?.volumeName);
  }
  ...
  get relatedVolumeSnapshotCounts() {
    const snapshots = this.$rootGetters[`${ PRODUCT_NAME }/all`](VOLUME_SNAPSHOT);
  ...
  get originalSnapshot() {
    if (this.spec?.dataSource) {
      return this.$rootGetters[`${ PRODUCT_NAME }/all`](VOLUME_SNAPSHOT).find((V) => V.metadata?.name === this.spec.dataSource.name);
    } else {
      return null;
    }
  }
  ...
  get relatedPV() {
    return this.$rootGetters[`${ PRODUCT_NAME }/all`](PV).find((pv) => pv.metadata?.name === this.spec?.volumeName);
  }
