...
export default class ClusterNode extends SteveModel {
  get _availableActions() {
    ...
    const setGPULabel = {
      action:  'setLabel',
      enabled: !this.gpuLabel,
      icon:    'icon icon icon-plus',
      label:   'Enable GPU Passthrough',
    };
    const removeGPULabel = {
      action:  'setLabel',
      enabled: this.gpuLabel,
      icon:    'icon icon icon-minus',
      label:   'Disable GPU Passthrough',
    };
  ...
  setLabel(resources = this) {
    this.$dispatch('promptModal', {
      resources,
      component: 'SetLabelDialog'
    });
  }
  ...
  get gpuLabel() {
    if (this.metadata?.labels?.['gpu.infortrend.com/vm-gpu-mode'] === 'true') {
      return true;
    } else {
      return false;
    }
    // return this.metadata?.labels?.['gpu.infortrend.com/vm-gpu-mode'];
  }
