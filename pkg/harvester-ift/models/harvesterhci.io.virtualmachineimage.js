...
export default class HciVmImage extends HarvesterResource {
  ...
  get uploadImage() {
    return async(file) => {
      ...
      try {
        this.$ctx.commit('cluster-common/uploadStart', this.metadata.name, { root: true });
        ...
        this.$ctx.commit('cluster-common/uploadEnd', this.metadata.name, { root: true });
  ...
  get preventDeletionMessage() {
    const nadOccupation = this.metadata.annotations[HCI_ANNOTATIONS.PVC_INUSE] ? JSON.parse(this.metadata.annotations[HCI_ANNOTATIONS.PVC_INUSE]) : '';

    if (nadOccupation?.status === 'True') {
      return 'This VM Image is InUse.';
    } else {
      return false;
    }
  }
