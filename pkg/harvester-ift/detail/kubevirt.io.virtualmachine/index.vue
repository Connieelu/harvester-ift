...
export default {
  ...
  async created() {
    ...
    const hash = {
      pods:     this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: POD }),
      services: this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: SERVICE }),
      events:   this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: EVENT }),
      allSSHs:  this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.SSH }),
      vmis:     this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.VMI }),
      // restore:  this.$store.dispatch(`${ PRODUCT_NAME }/findAll`, { type: HCI.RESTORE }),
    };

    await allHash(hash);

    this.showVmMetrics = await allDashboardsExist(this.$store, this.currentCluster.id, [VM_METRICS_DETAIL_URL], 'cluster');
  },
  ...
  computed: {
    ...
    allEvents() {
      ...
      return this.$store.getters[`${ PRODUCT_NAME }/all`](EVENT);
    },
    ...
    hasMetrics() {
      ...
      return !!this.$store.getters[`${ PRODUCT_NAME }/byId`]('service', 'cattle-monitoring-system/rancher-monitoring-grafana');
    },
  },
